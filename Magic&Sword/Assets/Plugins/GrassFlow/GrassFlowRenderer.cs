...
public class GrassFlowRenderer : MonoBehaviour {
    ...
    async Task InitAsync(bool isAsync = true) {
        ...
        try {
            ...
            await Task.Run(() => {
                if (!cameraCulls.ContainsKey(mainCam)) {
                    cameraCulls.Add(mainCam, new CameraCullResults(mainCam, terrainMeshes));
                }
            });
            ...
            var sortedMeshes = terrainMeshes.OrderByDescending(x => x.worldBounds.SqrDistance(camPos));

            foreach (var gMesh in sortedMeshes) {
    ...
    public async Task LoadChunksForMesh(GrassMesh gMesh) {
    ...
    void Render(ScriptableRenderContext context, Camera cam) {
        ...
        if (!cameraCulls.TryGetValue(cam, out cullResults)) {
            cullResults = new CameraCullResults(cam, terrainMeshes);
        ...
        if (cullResults.asyncCullTask == null || cullResults.asyncCullTask.IsCompleted) {

            cullResults.UpdatePos();

            cullResults.asyncCullTask = Task.Run(() => {
                RunCulling(cullResults);
            });
        }
    ...
    class CameraCullResults {
        ...
        public List<GrassCullChunk> culledChunksDblBfr = new List<GrassCullChunk>();

        public Task asyncCullTask;


        public CameraCullResults(Camera inCam, List<GrassMesh> gMeshes) {
            cam = inCam;
    ...
    void RunCulling(CameraCullResults cullResults) {



        if (cullResults.culledChunks == null) return;

        cullResults.culledChunksDblBfr.Clear();
        ...
        foreach (var gMesh in terrainMeshes) {
            ...
            foreach (var grassChunk in gMesh.chunks) {
                ...
                float camDist = grassChunk.worldBounds.SqrDistance(cullResults.pos);
                ...
                cullResults.culledChunksDblBfr.Add(chunk);
            }

            gMesh.shouldDraw = true;
        }

        var tmp = cullResults.culledChunks;
        cullResults.culledChunks = cullResults.culledChunksDblBfr;
        cullResults.culledChunksDblBfr = tmp;
    }
